...
**Статус на 06.09.2026:** реализована кодовая часть PERF-003a — атомарные claim,
heartbeat и integrity event для candidate/employee под выключенным по умолчанию
`SESSION_CONTROL_V2`. Активные операции выполняют один RPC. 36 новых автоматических
проверок включают исполнение SQL в PGlite и проверку серверного адаптера.
Применение миграции и конкурентные проверки на staging еще не выполнялись.
PERF-003 остается открытым: PERF-003b должен перенести autosave/finalize, remediation
и expiration check, сохранив валидацию всех типов ответов. Инструкция включения и
границы текущей реализации — `docs/18_SESSION_CONTROL_V2_ROLLOUT.md`.
