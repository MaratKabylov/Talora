...
**Статус на 10.09.2026:** локальные общий и поиндексный прогоны выполнены: 47 SELECT
shapes, 30 warm EXPLAIN до/после, девять индексов и четыре write proxies. Поиндексно
завершены 11 пар, включая два no-index контроля; шум записи не позволяет подтвердить
порог ≤10%. Миграции индексов не созданы. После сообщения пользователя о применении
