...
Приглашения HR-пользователей доступны владельцу на `/dashboard/company/members` после
применения миграции
`supabase/migrations/20260527130000_company_team_invitations_audit.sql`.

- У каждой организации может быть только один пользователь с ролью `owner`.
- Владелец приглашает пользователей с ролями `admin`, `recruiter` или `viewer` и
  выбирает организацию, которой будет выдан доступ.
- Если email уже существует в Talvia, доступ к организации активируется сразу.
- Если email еще не зарегистрирован, Supabase Auth отправляет письмо; пользователь
  завершает регистрацию на `/invite/company`, после чего доступ активируется.
- Приглашения, отключение доступа и изменения ролей сохраняются в
  `public.company_audit_logs` и видны владельцу организации.

Для email-приглашений организации разрешите callback URL приложения `/auth/confirm`
в настройках Supabase Auth. При отправке приглашения приложение добавляет переход на
`/invite/company` с идентификатором выбранной организации.

## Производительность runtime и локальная разработка

- Продуктовые замеры выполняйте на собранном приложении: `npm run build`, затем `npm run start`.
  Режим `npm run dev` включает компиляцию и диагностику разработки, поэтому его показатели не являются SLA.
- Размещайте Next.js runtime в том же регионе, что и основной Supabase Postgres, либо в ближайшем доступном
  регионе провайдера. Фактические регионы фиксируйте при каждом deployment в rollout evidence.
- Для локального сравнения режимов используйте `npm run perf:runtime -- --mode development --base-url
  http://127.0.0.1:<port>` или тот же вызов с `--mode production`. Скрипт обращается только к локальному origin и не сохраняет Supabase URL,
  project reference, ключи, токены или данные пользователей.
- На Windows при заметном замедлении из-за синхронизации `.next` или `node_modules` держите рабочую копию вне
  OneDrive, например в `C:\src\Talvia`, и устанавливайте зависимости отдельно в этой копии.
