...
import {
  ...
  archivePublishedTestVersionAction,
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <Link
                            ...
                            href={
                              isEditable && version.status === "draft"
                                ? `/dashboard/tests/${template.id}/builder?version=${version.id}`
                                : `/dashboard/tests/${template.id}/preview?version=${version.id}`
                            }
                          ...
                          version.versionNumber < latestPublishedVersionNumber ? (
                            <form action={archivePublishedTestVersionAction}>
                              <input name="templateId" type="hidden" value={template.id} />
                              <input name="versionId" type="hidden" value={version.id} />
      ...
      {archivedVersions.length > 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Архивные опубликованные версии остаются неизменяемыми и доступными в исторических
                  ...
                  {archivedVersions.map((version) => (
                    <tr className="border-t" key={version.id}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link
                          className={buttonVariants({ size: "sm", variant: "outline" })}
                          href={`/dashboard/tests/${template.id}/preview?version=${version.id}`}
                          target="_blank"
                        >
                          Preview
                        </Link>
        ...
        <Card>
          ...
          <CardContent className="space-y-5 pt-6">
            {process.env.BUILDER_SAVE_V2 === "true" ? <Link className={buttonVariants({ variant: "outline" })}
              href={`/dashboard/tests/${template.id}/builder?version=${draftVersion.id}`}>
              Изменить параметры в конструкторе
            </Link> : <form action={updateTestVersionAction} className="space-y-5">
              <input name="templateId" type="hidden" value={template.id} />
              ...
              <Link
                ...
                href={
                  draftVersion && isEditable
                    ? `/dashboard/tests/${template.id}/builder?version=${draftVersion.id}`
                    : `/dashboard/tests/${template.id}/preview?version=${template.latestVersion.id}`
                }
